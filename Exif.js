...
EXIF.main = function() {
	...
	function findEXIFinJPEG(file) {
        ...
        while (offset < length) {
            ...
            if (marker == 22400) {
                if (debug) Ti.API.log("Found 0xFFE1 marker");
    ...
    function readEXIFData(file, start) {
        ...
        if (file.getLongAt(tiffOffset+4, bigEnd) != 0x00000008) {
            if (debug) Ti.API.log("Not valid TIFF data! (First offset not 8)", file.getShortAt(tiffOffset+4, bigEnd));
            //return false;
        }
...
EXIF.FileReader = function(filepath) {
	if (typeof filepath == "string") {
		console.log("EXIF::FileReader Condition 1");
		var filestream = Ti.Filesystem.openStream(Ti.Filesystem.MODE_READ, filepath);		
	} else if (filepath.hasOwnProperty("media")) {
		console.log("EXIF::FileReader Condition 2");
		filestream = Ti.Stream.createStream(
			{
				mode : 	Ti.Filesystem.MODE_READ,
				source : filepath.media
			}
		);
	}
	...
	var buffer = Ti.createBuffer(
		{
			length:65536, //EXIF is in the 1s 64k of any file
	...
	this.getByteAt = function($offset) {
		...
		if (Ti.Platform.osname == "android") {
			value = value & 0xFF; //Turned Signed Byte into Unsigned Byte (Java Bytes are -128 to 128, rather than 0 to 255 = dumb)
		}
		return value;
	};
	this.getShortAt = function($offset, bigEndian) {
		var byteOrder = bigEndian == true ? Ti.Codec.BIG_ENDIAN : Ti.Codec.LITTLE_ENDIAN;
		var value = Ti.Codec.decodeNumber({
			source : buffer,
			position : $offset,
		...
		if (Ti.Platform.osname == "android") {
			value = value & 0xFFFF; //Turned Signed Byte into Unsigned Byte
		}
		return value;
	};
	this.getLongAt = function($offset, bigEndian) {
		...
		if (Ti.Platform.osname == "android") {
			value = value & 0xFFFFFFFF; //Turned Signed Byte into Unsigned Byte
		}
		return value;
	};
	this.getSLongAt = this.getIntAt = function($offset, bigEndian) {
		var byteOrder = bigEndian == true ? Ti.Codec.BIG_ENDIAN : Ti.Codec.LITTLE_ENDIAN;
		var value = Ti.Codec.decodeNumber({
			source : buffer,
			position : $offset,
	...
	this.getFloatAt = function($offset, bigEndian) {
		var byteOrder = bigEndian == true ? Ti.Codec.BIG_ENDIAN : Ti.Codec.LITTLE_ENDIAN;
		var value = Ti.Codec.decodeNumber({
			source : buffer,
			position : $offset,
	...
	this.getDoubleAt = function($offset, bigEndian) {
		var byteOrder = bigEndian == true ? Ti.Codec.BIG_ENDIAN : Ti.Codec.LITTLE_ENDIAN;
		var value = Ti.Codec.decodeNumber({
			source : buffer,
			position : $offset,
...
EXIF.fromPath = function(filepath) {
	...
	for (var props in data) {
		//Ti.API.log("Exif: " + props + " => " + data[props]);
	}
	return data;
};
